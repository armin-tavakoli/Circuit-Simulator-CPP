...
void ACVoltageSource::print() const { cout << "Type: AC Source, Name: " << name << ", Nodes: (" << node1 << "," << node2 << "), Mag=" << ac_magnitude << "V, Phase=" << ac_phase << "deg" << endl; }
string ACVoltageSource::toNetlistString() const { return name + " " + to_string(node1) + " " + to_string(node2) + " AC " + to_string(ac_magnitude) + " " + to_string(ac_phase); }
...
void SinusoidalVoltageSource::stampAC(MatrixXcd& A, VectorXcd& b, int current_idx, double omega) const {
    int n1 = node1 - 1;
    int n2 = node2 - 1;
    if (n1 >= 0) A(current_idx, n1) = 1.0;
    if (n2 >= 0) A(current_idx, n2) = -1.0;
    b(current_idx) = this->v_offset;
    if (n1 >= 0) A(n1, current_idx) = 1.0;
    if (n2 >= 0) A(n2, current_idx) = -1.0;
}
...
void PulseVoltageSource::stampAC(MatrixXcd& A, VectorXcd& b, int current_idx, double omega) const {
    int n1 = node1 - 1;
    int n2 = node2 - 1;
    if (n1 >= 0) A(current_idx, n1) = 1.0;
    if (n2 >= 0) A(current_idx, n2) = -1.0;
    b(current_idx) = this->v_initial;
    if (n1 >= 0) A(n1, current_idx) = 1.0;
    if (n2 >= 0) A(n2, current_idx) = -1.0;
}
...
void Diode::stampAC(MatrixXcd& A, VectorXcd& b, int current_idx, double omega) const {}
